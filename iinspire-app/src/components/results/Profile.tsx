...
const Profile = () => {
  ...
  useEffect(() => {
    if (myData) {
      console.log("📊 Most Recent Result (myData):");
      console.log(myData.groupAverages);
    }
  
    if (selectedComparison) {
      console.log("🕓 Selected Comparison Result:");
      console.log(selectedComparison.groupAverages);
    }
  }, [myData, selectedComparison]);
        ...
        .then((data) => {
          if (data.Results && data.Results.length > 0) {
            ...
            const sortedResults = data.Results.sort(
              (a: any, b: any) =>
                new Date(b.dataCreated).getTime() -
                new Date(a.dataCreated).getTime()
            );
            const latest = sortedResults[0];
            const previous = sortedResults.slice(1);
            // Set latest survey as main dataset
            setSurveyDate(latest.dataCreated);
            setMyData({
              groupAverages: {
                group_1_average: latest.civicEngagement,
                group_2_average: latest.stemInterest,
                group_3_average: latest.stemEfficacy,
                group_4_average: latest.stemOutcome,
                group_5_average: latest.researchOutcome,
                group_6_average: latest.researchEfficacy,
              },
            });
            // Map the rest into DataSet[] with timestamps for dropdown
            const pastDataSets = previous.map((entry: any) => ({
              groupAverages: {
                group_1_average: entry.civicEngagement,
                group_2_average: entry.stemInterest,
                group_3_average: entry.stemEfficacy,
                group_4_average: entry.stemOutcome,
                group_5_average: entry.researchOutcome,
                group_6_average: entry.researchEfficacy,
              },
              dataCreated: entry.dataCreated, // for labeling
            }));
            setComparisonResults(pastDataSets);
          }
        })
        .catch((error) => {
  ...
  const handleExportToPDF = async () => {
    ...
    doc.setFontSize(22);
    doc.text(`${possessiveName} MySTEMGrowth Profile`, marginLeft, y);
    y += 30;
    ...
    const cardSections = [
      {
        ...
        color: "#bf5fff",
        text: cardText.civicEngagement,
      },
      {
        title: "STEM Interests",
        color: "#4a86e8",
        text: cardText.stemInterests,
      },
      {
        title: "STEM Self Efficacy",
        color: "#ff0000",
        text: cardText.stemSelfEfficacy,
      },
      {
        title: "STEM Outcome Expectations",
        color: "#ff9900",
        text: cardText.stemOutcomeExpectations,
      },
      {
        title: "Research Outcome Expectations",
        color: "#93c47d",
        text: cardText.researchOutcomeExpectations,
      },
      {
        title: "Research Self Efficacy",
        color: "#fff16e",
        text: cardText.researchSelfEfficacy,
      },
    ];
  ...
  return (
    <div>
      <div className="profile-container">
        <h1 className="header-profile">MySTEMGrowth Profile</h1>
        <button onClick={handleExportToPDF} className="export-button">
          Export to PDF
        </button>
        {comparisonResults.length > 0 && (
          <div className="comparison-dropdown">
            <label htmlFor="comparisonSelect">Compare to past result:</label>
            <select
              id="comparisonSelect"
              onChange={(e) => {
                const index = parseInt(e.target.value);
                setSelectedComparison(
                  isNaN(index) ? null : comparisonResults[index]
                );
              }}
            >
              <option value="">None</option>
              {comparisonResults.map((result: any, i: any) => (
                <option key={i} value={i}>
                  {result.dataCreated
                    ? new Date(result.dataCreated).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                      })
        ...
        {isClient && (
          <div className="card-container">
            <Card
              ...
              moreInfo={cardText.civicEngagement}
              color="#bf5fff"
            />
            <Card
              id="Card2"
              title="STEM Interests"
              description="Your interest in STEM-related activities."
              moreInfo={cardText.stemInterests}
              color="#4a86e8"
            />
            <Card
              id="Card3"
              title="STEM Self Efficacy"
              description="Your confidence in STEM-related tasks."
              moreInfo={cardText.stemSelfEfficacy}
              color="#ff0000"
            />
            <Card
              id="Card4"
              ...
              moreInfo={cardText.stemOutcomeExpectations}
              color="#ff9900"
            />
            <Card
              id="Card5"
              ...
              moreInfo={cardText.researchOutcomeExpectations}
              color="#93c47d"
            />
            <Card
              id="Card6"
              ...
              moreInfo={cardText.researchSelfEfficacy}
              color="#fff16e"
            />
          </div>
        )}
